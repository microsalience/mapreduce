 ...
 * 
 */
public class AccessLogMapperTest extends
...
//		if (!"2000-00-00".equalsIgnoreCase(array[1])) {
//			if (array[0].split(COLUMN_SPLIT).length != 24) {
//				array[1] = "20000000";
//			}
//		} 
	...
	private String[] parseUrl(String str) {
		...
		try {
			...
			/**String client_time = EMPTY_CHAR;
			index = request.indexOf("&time=");
			if (index >= 0) {
				String s = request.substring(index + 6);
				client_time = cleanString(s);
			}
			String method = EMPTY_CHAR;
			index = request.indexOf("&method=");
			if (index >= 0) {
				String s = request.substring(index + 8);
				method = cleanString(s);
			}*/
			...
			String visit_time = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(date);
			...
			sb.append(id).append(split).append(site).append(split)
					...
					.append(visit_time).append(split).append(status).append(split)
					.append(client).append(split).append(sfrom).append(split)
					.append(sfrom_www).append(split).append(lady).append(split)
					.append(protocol).append(split).append(f_argv).append(split)
					.append(anchor).append(split).append(callback).append(split)
					.append(hahapoint).append(split).append(refer).append(split)
	...
	public static void main(String args[]) throws Exception {
		AccessLogMapperTest alm = new AccessLogMapperTest();
